...
const initailSocialData = appsList.filter(each =>each.category === "SOCIAL")
//   console.log(initailSocialData)

const AppStore = () => {
    const [appData , setAppData] = useState(initailSocialData)
    const [avtiveTab,setActiveTab] = useState("SOCIAL")
    ...
    const getUserSearchApp = (event) =>{
        setSearchInput(event.target.value)
    }

     const searchResultData = appData.filter(search =>search.appName.toLowerCase().includes(searchInput.toLowerCase()))

  return (
    <div className='app-store-main-container'>
        <h1 className='app-store-heading'>App Store</h1>
        <div className='app-store-search-bar-container'>
            <input type="search" className='app-search-bar' onChange={getUserSearchApp}/>
        ...
        <ul className='app-tabs-container'>
            {tabsList.map(eachTab =>(
                <li key={eachTab.tabId} className={avtiveTab === eachTab.tabId ? "app-tab-list-item tick":"app-tab-list-item"}>
                    <button className='app-tab-button' onClick={() =>getActiveTabId(eachTab.tabId)}>{eachTab.displayText}</button>
                </li>
            ))}
        </ul>
        <div className='apps-container'>
            <ul className='app-list-container'>
                {searchResultData.map(app =>(
                    <li key={app.appId} className="app-card">
                        <img src={app.imageUrl} alt={app.appName} className="app-logos"/>
